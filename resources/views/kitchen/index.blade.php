<x-kitchen-layout>
    ...
    <div class="flex flex-col h-screen bg-gray-200 font-sans">
        <header class="flex justify-between items-center bg-white shadow-md p-2">
            <h1 class="text-2xl font-bold text-gray-800 px-4">Cocina</h1>
            <x-user-dropdown />
        </header>
        ...
        <div class="flex flex-1 overflow-hidden">
            ...
            <div class="w-1/4 bg-gray-100 border-r border-gray-300 flex flex-col">
                <div class="p-4 border-b border-gray-300 bg-white">
                    <h2 class="text-xl font-bold text-center text-gray-800">Pendidos Pendientes</h2>
                </div>
                <div class="flex-order pending-orders flex p-3 space-y-3">
            ...
            <div class="w-1/2 bg-white flex flex-col">
                <div class="p-4 border-b border-gray-200">
                    <h2 class="text-xl font-bold text-center text-gray-800">En Progreso</h2>
                </div>
            ...
            <div class="w-1/4 bg-gray-100 border-l border-gray-300 flex flex-col">
                <div class="p-4 border-b border-gray-300 bg-white">
                    <h2 class="text-xl font-bold text-center text-gray-800">Total a Preparar</h2>
                </div>
                <div class="items-counts flex-1 overflow-y-auto p-3 space-y-4">
                    <div class="dine-in bg-white border border-gray-200 rounded-lg shadow-sm">
                        <h3 class="text-lg font-semibold p-3 bg-gray-50 rounded-t-lg border-b">Cantidad</h3>
                        <div class="p-3 items space-y-1">
                            {{-- JS will populate this --}}
                        </div>
                    </div>
                    <div class="take-away bg-white border border-gray-200 rounded-lg shadow-sm">
                        <h3 class="text-lg font-semibold p-3 bg-gray-50 rounded-t-lg border-b">Otros</h3>
                        <div class="p-3 items space-y-1">
                            {{-- JS will populate this --}}
                        </div>
                    </div>
                    <div class="completed-orders bg-white border border-gray-200 rounded-lg shadow-sm">
            <h3 class="text-lg font-semibold p-3 bg-gray-50 rounded-t-lg border-b">
        Últimos completados
            </h3>
         <div class="p-3 completed-orders-list space-y-1">
        <p class="text-sm text-gray-500">Sin órdenes.</p>
         </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    console.log('✅ Kitchen polling de cancelaciones activo');

    function checkCancelledOrders() {

        // revisa TODAS las órdenes visibles (pendientes + en progreso)
        $('.order-item').each(function () {

            const orderElement = $(this);
            const orderId = orderElement.attr('id')?.replace('order', '');

            if (!orderId) return;

            fetch(`/kitchen/check-order/${orderId}`)
                .then(r => r.json())
                .then(data => {
                    if (!data.active) {

                        console.log('❌ Orden cancelada externamente:', orderId);

                        orderElement.fadeOut(200, function () {
                            $(this).remove();
                            calculateMenuSumByType();
                        });
                    }
                ...
                .catch(() => {
                    // silencioso para no spamear consola
                });
        });
    }

    // cada 3 segundos (igual que antes, pero sin reload)
    setInterval(checkCancelledOrders, 3000);
});
</script>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function refreshCompletedOrders() {
        ...
        fetch('/kitchen/completed-orders')
            .then(r => r.json())
            .then(data => {

                const container = document.querySelector('.completed-orders-list');
                if (!container) return;
                ...
                data.forEach(order => {
                    ...
                    block.className =
                        'border-b border-gray-200 pb-2 mb-2 last:border-0';
    ...
    setInterval(refreshCompletedOrders, 3000);
});
</script>
